...
export async function PUT(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const backendResponse = await fetch(buildBackendUrl("/admin/users/update.php"), {
      method: "PUT",
      headers: {
...
export async function DELETE(request: Request, context: RouteContext) {
  ...
  try {
    const { id } = await context.params;
    const backendResponse = await fetch(buildBackendUrl(`/admin/users/delete.php?id=${encodeURIComponent(id)}`), {
      method: "DELETE",
      headers: {
        Authorization: authHeader,
      },
